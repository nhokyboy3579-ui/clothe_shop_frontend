...
import { UserSaleService } from "@/services/UserSaleService"; // 1. Import Service Sale
...
export default function ShopPage() {
  ...
  const [tempPrice, setTempPrice] = useState([MIN_PRICE, MAX_PRICE]);

  // --- PHÂN TRANG ---
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 8;

  // 1. Tải dữ liệu gộp từ cả 2 nguồn
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Gọi song song 2 API
        const [resNormal, resSale] = await Promise.all([
          UserProductService.getAllActive(),
          UserSaleService.getActiveSales(),
        ]);

        // Xử lý gộp dữ liệu để không bị trùng lặp ID
        const combinedMap = new Map();

        // Đưa hàng thường vào Map trước
        resNormal.forEach((p) => combinedMap.set(p.id, p));

        // Đưa hàng sale vào Map (nếu trùng ID sẽ ghi đè bằng dữ liệu Sale)
        resSale.forEach((p) => {
          combinedMap.set(p.id, {
            ...p,
            // Đồng bộ key giá giữa 2 Model để logic lọc bên dưới chạy đúng
            sale_price: p.price_sale || p.sale_price,
            price: p.price_original || p.price,
          });
        });
        ...
        const uniqueCats = [
          ...new Set(combinedData.map((p) => p.category_name)),
        ].filter(Boolean);
  ...
  // 2. Logic Lọc & Sắp xếp (GIỮ NGUYÊN NHƯ BAN ĐẦU)
  const filteredProducts = useMemo(() => {
    ...
    switch (sortOption) {
      ...
      case "price_asc":
        result.sort((a, b) => {
          const pA =
            a.sale_price && a.sale_price < a.price ? a.sale_price : a.price;
          const pB =
            b.sale_price && b.sale_price < b.price ? b.sale_price : b.price;
          return pA - pB;
        });
        break;
      case "price_desc":
      ...
      <div className="bg-white shadow-sm py-12 mb-8 border-b border-gray-100">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-gray-500 max-w-lg mx-auto">
            Tất cả sản phẩm bao gồm cả ưu đãi mới nhất.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 flex flex-col lg:flex-row gap-8 items-start">
        <aside className="w-full lg:w-1/4 lg:sticky lg:top-24 z-10 space-y-6 h-fit">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <div className="mb-6">
              <h3 className="font-bold text-gray-900 mb-3 uppercase text-xs tracking-wider">
                Tìm kiếm
              </h3>
...
function ShopProductCard({ product }) {
  ...
  return (
    <Link
      ...
      className="group bg-white rounded-xl overflow-hidden border border-transparent hover:border-gray-200 hover:shadow-xl transition-all duration-300 flex flex-col h-full"
    >
      <div className="relative aspect-[3/4] w-full bg-gray-100 overflow-hidden">
        ...
        />
        {hasSale && (
          <div className="absolute top-2 left-2 bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm tracking-wide">
            -{discount}%
          </div>
        )}
      </div>

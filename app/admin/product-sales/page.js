...
export default function AdminProductSales() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const existingProductIds = Array.isArray(salesData) ? salesData.map(s => s.product_id) : [];
  ...
  useEffect(() => { fetchData(); }, [currentPage, debouncedSearchTerm]);

  const selectedProductInfo = allProducts.find(p => String(p.id) === String(formData.product_id));
  ...
  const handleSubmit = async (e) => {
    ...
    const currentPrice = selectedProductInfo?.price_buy || selectedProductInfo?.price || 0;
    ...
    try {
      ...
      toast.error('Vui lòng kiểm tra lại dữ liệu', { id: toastId });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-6 border-b pb-4">
        ...
        <button onClick={handleOpenAdd} className="bg-green-600 text-white px-4 py-2 rounded font-bold hover:bg-green-700 shadow">+ Thêm Giảm giá</button>
      </div>
      ...
      <div className="mb-4">
          <input
              ...
              className="w-full sm:w-80 p-2 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-green-500"
              value={searchTerm} 
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y text-sm">
            {!loading && sales.map(s => {
                ...
                let statusBadge;
                if (s.status !== 1) {
                    statusBadge = <span className="text-gray-500 bg-gray-100 px-2 py-1 rounded text-xs font-bold">Tạm ẩn</span>;
                } else if (now < start) {
                    statusBadge = <span className="text-blue-600 bg-blue-100 px-2 py-1 rounded text-xs font-bold">Sắp bắt đầu</span>;
                } else if (now >= start && now <= end) {
                    statusBadge = <span className="text-green-600 bg-green-100 px-2 py-1 rounded text-xs font-bold">Đang chạy</span>;
                } else {
                    statusBadge = <span className="text-red-500 bg-red-50 px-2 py-1 rounded text-xs font-bold">Hết hạn</span>;
                }

                return (
                    <tr key={s.id} className="hover:bg-gray-50">
                        <td className="p-4 font-mono">{s.id}</td>
                        <td className="p-4 font-bold">{s.product?.name || 'N/A'}</td>
                        <td className="p-4 text-right line-through text-gray-400">{formatCurrency(s.product?.price_buy || s.product?.price)}</td>
                        ...
                        <td className="p-4 text-center">
                            <button onClick={() => handleOpenEdit(s)} className="text-blue-600 font-bold mr-3">Sửa</button>
                            <button onClick={() => handleDelete(s.id)} className="text-red-600 font-bold">Xóa</button>
                        </td>
                    </tr>
                );
            })}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg overflow-hidden transform transition-all max-h-[90vh] overflow-y-auto">
            <div className="bg-gray-800 text-white px-6 py-4 flex justify-between items-center sticky top-0">
              <h3 className="font-bold uppercase">{isEditing ? 'Cập nhật giảm giá' : 'Thêm mới giảm giá'}</h3>
              <button onClick={() => setShowModal(false)} className="text-2xl hover:text-gray-300">&times;</button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-bold mb-1">Sản phẩm áp dụng (*)</label>
                <select className="w-full border p-2.5 rounded bg-white" required value={formData.product_id} 
                  onChange={(e) => setFormData({...formData, product_id: e.target.value})} disabled={isEditing}>
                  <option value="">-- Chọn Sản phẩm --</option>
                  {isEditing ? (
                    <option value={currentSaleProduct?.id}>{currentSaleProduct?.name}</option>
                  ) : (
                ...
                {selectedProductInfo && (
                    <div className="mt-3 p-3 bg-blue-50 text-blue-900 rounded border border-blue-200 flex justify-between items-center text-sm shadow-sm animate-[fadeIn_0.3s]">
                        ...
                        <span className="font-bold text-lg">{formatCurrency(selectedProductInfo.price_buy || selectedProductInfo.price || 0)}</span>
                    </div>
                )}
              </div>
              <div>
                <label className="block text-sm font-bold mb-1">Giá giảm (VND) (*)</label>
                <input type="number" step="1000" className="w-full border p-2.5 rounded" required 
                  value={formData.price_sale} onChange={(e) => setFormData({...formData, price_sale: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                  <div>
                      <label className="block text-sm font-bold mb-1">Ngày Bắt đầu</label>
                      <input type="date" className="w-full border p-2.5 rounded" required 
                          value={formData.date_begin} onChange={(e) => setFormData({...formData, date_begin: e.target.value})} />
                  </div>
                  <div>
                      <label className="block text-sm font-bold mb-1">Ngày Kết thúc</label>
                      <input type="date" className="w-full border p-2.5 rounded" required 
              ...
              <div>
                ...
                <select className="w-full border p-2.5 rounded bg-white" value={formData.status} 
              ...
              <div className="flex justify-end gap-3 pt-6 border-t mt-6">
                <button type="button" onClick={() => setShowModal(false)} className="px-5 py-2.5 text-gray-600 hover:bg-gray-100 rounded font-bold">Hủy</button>
                <button type="submit" disabled={isSubmitting} className="px-6 py-2.5 bg-green-600 text-white rounded font-bold hover:bg-green-700 shadow-lg disabled:bg-gray-400">
